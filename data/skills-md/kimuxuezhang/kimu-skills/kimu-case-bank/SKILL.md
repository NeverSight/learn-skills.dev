---
name: kimu-case-bank
description: 检索、选择、核验并维护商业、内容、课程与个人实践案例；公开层只保存可公开核验的案例，用户材料和敏感案例保存在本机私有层。用户要求查找案例、收录材料、核对证据或建立个人案例库时使用。
---

# Kimu 案例库

为所有 Kimu Skill 提供真实案例，同时把可公开分享的材料与用户私有材料严格分开。公开层必须能让仓库使用者回到公开来源核验；个人经历、内部项目、客户或学员反馈和业务数据只进入本机私有层。

用户可以输入：

```text
Kimu 案例库：为这个观点找一个可核验案例
Kimu 案例库：把这份客户反馈收入我的私有案例库
```
...
- 关键事实已经回到原始来源核对；
- 不包含未经授权的客户、学员、未成年人、收入或内部经营信息；
- 用户明确要求把它加入可分享的公共案例库。

### 私有层

默认位置是 `~/.kimu/case-bank/`，编号使用 `KCB-Uxxx`。用于保存用户自己的经历、项目资料、咨询记录、客户或学员案例和业务数据。该目录独立于 GitHub 技能包，不得复制、打包或上传到公开仓库。
...
1. 用户已授权读取、且直接证明当前判断的私有项目、客户或学员案例；
2. 可公开核验、且与当前问题直接相关的商业、内容、课程或个人实践案例；
3. 可公开核验的传播、管理、技术、历史或跨行业案例；
