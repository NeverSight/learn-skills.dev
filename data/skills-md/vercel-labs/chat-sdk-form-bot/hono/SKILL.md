...
Hono apps deploy to Vercel with **zero configuration**. Your server routes automatically become [Vercel Functions](https://vercel.com/docs/functions) using [Fluid compute](https://vercel.com/docs/fluid-compute) by default.
...
Use [Vercel CLI](https://vercel.com/docs/cli/dev) to run locally with the same behavior as production:
...
- **Vercel Routing Middleware** — Runs at the edge before your app is invoked (rewrites, redirects, headers). See [Vercel Routing Middleware docs](https://vercel.com/docs/routing-middleware).
...
// With TypeScript generics for request-scoped variables
type Env = {
  Variables: { user: User }
}
