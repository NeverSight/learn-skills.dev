...
**权限边界（永���不可突破）**：
1. 以招标文件为唯一事实来源，**禁止编造**企业资质、项目案例、产品参数、人员、获奖、价格、交期、厂家承诺；
2. 不做投标决策、不做最终合规终审、不承诺中标；关键定稿、正/负偏离判定、评分取舍权归人工用户；
3. 不擅自跳过用户确认环节自行推进流程；
4. **风控台账内容（分值、废标条款、硬约束值）严禁进入正文**，撰写正文时严禁加载风控台账；
5. 内部台账/评审笔记中未知项标 `待确认`；**正式正文里绝不留** `TODO / 待确认 / 待补充 / 公司名称 / 项目名称：填写` 等草稿痕迹——无法提供的证据改为引用其他正式章节或附件（如"详见本投标文件《项目管理人员名单》"）。
...
1. **文档转 Markdown 便于处理**：
   - `.doc/.docx` → `pandoc` 或 `scripts/convert_to_md.py`
   - `.pdf` → `scripts/convert_to_md.py`（文本型）；扫描件先 OCR
   - 图片/扫描件 → 多模态直接读取或 OCR
...
若为**工程技术咨询/工程监测（轨道交通第三方监测、地铁控制保护区“地保”监测、运营期变形监测、基坑/隘道监测）/工程测量测绘**，**加载 `references/engineering-bid.md`**（监测标 16 章标准格式、影响分区分级、数据闭环、报警值、应急、监测专家评审；**测量测绘标专属章节骨架见第八节**；品牌隔离、工程资格/证据、报价工作流）。若为纯施工类，施工组织设计、工程量清单、施工进度需另配施工标流程。行业细分（IT/建筑/医疗/教育/制造/物流/咨询）关注点见 `references/industry-guides.md`。
...
- **撰写提示词.md**（供步骤⑤撰稿）：分概述型/详细设计型/逐条响应型；逐条响应型绑定 T 编号，含招标原文/参数要求/响应方式/响应内容/字数/段落结构/图表要求；**严禁**含评分分值、废标条款、"必须/不得"硬约束值。
- **风控台账.md**（仅供步骤⑦质检）：每条含对应评分条目+分值、招标技术硬性约束、废标风险提示；**严禁**进入正文、严禁复制进提示词。
...
- **降 AI 味**：避免"全过程、全链路、全角色、全闭环"式对仗口号；多用具体业���场景、数据流、接口联调、角色协作、质量记录、验收材料；
- 图表用 Markdown/HTML（需 SVG 架构图的长技术章节优先 HTML，便于后续转 docx）；
- 不确定信息**不编造**：可验证事实（资质/案例/人名/参数）用占位符 `{公司名称}` `{产品名}` `{案例项目名}` 或引用正式附件；
...
| 何时读 | 文件 |
| --- | --- |
| 步骤① ��洗与三分法细则 | `references/parsing-rules.md` |
...
| 脚本 | 功能 |
| --- | --- |
| `scripts/convert_to_md.py` | PDF/DOCX/DOC/TXT/XLSX → Markdown（便于 AI 读取） |
...
| `scripts/monitoring_fee_estimate.py` | 工程监测报价估算（2002标准：点次/米次→技术费→折扣） |
| `scripts/build_docx.py` | 单份标书 Markdown → DOCX（政府/企业/公路格式预设） |
| `scripts/build_bid_package.py` | 一键整合：多章节+（可选）报价明细 → 单个投标 DOCX |

> 完整可运行样例（走完九步流程、含 00~06 产物与整合 DOCX）：`examples/地铁保护区第三方监测示例/`，说明见 `examples/README.md`。
...
| 步骤① | `执行步骤1：解析招标文件，项��名：XX，招标文件路径：XXX` |
| 步骤②–④ | `执行步骤2/3/4` |
| 步骤⑤ | `执行步骤5：逐章撰写` |
| 步骤⑦ | `执行步骤7：合规质检+专家评审` |
