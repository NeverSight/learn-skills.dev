---
name: deep-research
description: "Use when a mathematical modeling task needs rigorous literature retrieval, source verification, method comparison, fact-checking, contradiction analysis, or a reproducible evidence matrix before method selection or paper citation."
metadata:
  version: "2.11.0"
...
`references/math-modeling-adaptation.md` before running any mode or dispatching any agent. That
adaptation is authoritative when it conflicts with the generic academic workflow below. Keep this
skill subordinate to `workflow-orchestrator`; do not invoke `academic-pipeline`.
