---
name: 6verity
description: "数学建模竞赛最终验证和验收阶段，支持 Typst 和 LaTeX 双引擎。用于论文写完后检查章节数量、标题顺序、图表引用、数值一致性、参考文献、代码可复现性、支撑材料 ZIP 与文件清单、编译和提交就绪状态。"
...
8. 如需提交支撑材料：支撑材料源目录、最终 ZIP、ZIP 内的 `支撑材料文件清单.md`，以及论文附录中的支撑材料文件列表。
...
### Step 9: 验收支撑材料 ZIP 和文件清单

如需提交支撑材料，必须执行以下检查；任一硬门禁失败都不得进入提交状态：

1. 确认 ZIP 由 7-Zip 生成且只有一个支撑材料压缩包，论文 PDF 与 ZIP 分开。
2. 使用 `7z t "$SUPPORTING_ZIP"` 检查完整性，要求退出码为 0。
3. 检查精确字节数，要求 ZIP 小于 `20 * 1024 * 1024` 字节；建议控制在 19.5 MiB 以下。
4. 解压到新的临时目录，从解压副本读取 `支撑材料文件清单.md`，逐项核对相对路径、文件数、字节数和 SHA-256。ZIP 条目应恰好等于清单记录的材料文件加清单文件自身；清单不对自身做 SHA-256 自校验。
5. 比较 ZIP、清单与论文附录：论文附录应列出清单中的全部材料文件，并额外列出 `支撑材料文件清单.md` 自身，因此与 ZIP 的完整条目集合一致。
6. 确认全部可运行源程序同时存在于论文附录和 ZIP；从解压副本执行轻量复现测试。
7. 使用 AI 时确认 ZIP 包含可打开的 `AI 工具使用详情.pdf`，且内容与论文声明一致。
8. 检查 ZIP 内没有承诺书、编号专用页、论文 PDF、队员姓名、学校、队号、赛区、联系方式或其他身份信息；同时检查文件夹名、文件名、文档属性和图片元数据。
9. 删除缓存、虚拟环境、临时文件、编译产物和无关重复文件；但不得为了缩小体积删除复现论文结论所需的代码、外部数据或证据。
10. 记录最终论文 PDF 和 ZIP 的文件大小及哈希。关闭文件后再用官方客户端生成 MD5；MD5 冻结后不得重新打开保存或重新压缩。

若确实没有支撑材料，确认论文附录明确写有“本论文没有支撑材料”，并且没有创建或提交空 ZIP。
...
- 需要支撑材料但 ZIP 缺失、损坏、不是单个 ZIP，或大小达到/超过 20 MiB。
- ZIP 缺少 `支撑材料文件清单.md`，或 ZIP 内容、清单与论文附录文件列表不一致。
- 必要源程序未同时出现在论文附录和 ZIP，或从解压副本无法运行必要程序。
- 使用 AI 但 ZIP 缺少 `AI 工具使用详情.pdf`，或其内容与论文声明不一致。
- ZIP 含承诺书、编号专用页、论文 PDF 或任何队伍、学校、赛区身份信息。
