...
- **pass_criteria**:
  ...
  - The required-figure matrix includes one paper-overview schematic. Every important algorithm has at least one selected diagram role—algorithm flowchart, concept/mechanism diagram, or both—chosen by explanation load, with editable Draw.io source, final export, clear non-crossing routes, and `PASS` QA before G5 can pass.
