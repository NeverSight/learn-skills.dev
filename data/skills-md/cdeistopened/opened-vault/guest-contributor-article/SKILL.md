...
**Publication:**
- Publish on OpenEd blog via Webflow API with full byline (see Webflow HTML Rules below)
- Create social assets via `newsletter-to-social`
- Send contributor share-ready posts with @handles

---

## Webflow Rich Text HTML Rules

**CRITICAL: Webflow's CMS rich text field silently drops or fails to render certain HTML. These rules are non-negotiable when pushing content via the API.**

### Never use `<ul>` or `<li>` tags

Webflow's rich text renderer strips list items pushed via API even when the API accepts and stores them. The content will appear in the CMS editor but **will not render on the published page**.

**Instead:** Convert all list items to `<p>` tags with `<strong>` for the lead-in.

```
# WRONG - will be silently stripped on render
<ul><li><strong>Dog Man</strong> (Dav Pilkey) - Description...</li></ul>

# RIGHT - always renders
<p><strong>Dog Man</strong> (Dav Pilkey) - Description...</p>
```

### Image figures must use Webflow's class structure

```html
<figure class="w-richtext-figure-type-image w-richtext-align-center"
  style="max-width:65%;margin:2em auto"
  data-rt-type="image" data-rt-align="center">
  <div><img src="[CDN_URL]" alt="[descriptive alt text]" loading="lazy"></div>
</figure>
```

- Use `w-richtext-align-center` for inline images (not `floatleft`/`floatright` which cause awkward text wrapping)
- Set `max-width` to control size (65% is a good default for infographics)
- Always include `alt` text and `loading="lazy"`

### Safe HTML tags (confirmed working)

| Tag | Use |
|-----|-----|
| `<p>` | All body text, including items that would normally be list items |
| `<h2>`, `<h3>` | Section and subsection headings |
| `<strong>` | Bold text |
| `<em>` | Italic text |
| `<a href="...">` | Links (internal and external) |
| `<figure>` | Images (with Webflow classes above) |

### Tags to avoid

| Tag | Problem | Alternative |
|-----|---------|-------------|
| `<ul>`, `<ol>`, `<li>` | Silently stripped on render | Use `<p>` with `<strong>` lead-ins |
| `<div>` (standalone) | May be stripped | Use `<p>` |
| `<br>` | Inconsistent | Use separate `<p>` tags |
| Inline `style` on `<p>` | May be stripped | Rely on Webflow's rich text class styling |

### Markdown-to-Webflow conversion function

When converting markdown to Webflow HTML, use this pattern:

```python
def markdown_to_webflow_html(md_text):
    """Convert markdown to Webflow-safe rich text HTML.

    Key rule: NO <ul>/<li> tags. All items become <p> tags.
    """
    # Strip YAML frontmatter
    md_text = re.sub(r'^---\n.*?\n---\n', '', md_text, flags=re.DOTALL)
    # Strip H1 (becomes CMS 'name' field)
    ...
    for line in lines:
        ...
        if stripped.startswith('### '):
            html_parts.append('<h3>%s</h3>' % inline_format(stripped[4:]))
        elif stripped.startswith('## '):
            html_parts.append('<h2>%s</h2>' % inline_format(stripped[3:]))
        elif stripped.startswith('- ') or stripped.startswith('* '):
            # List items become paragraphs
            html_parts.append('<p>%s</p>' % inline_format(stripped[2:]))
        else:
            html_parts.append('<p>%s</p>' % inline_format(stripped))

    return '\n'.join(html_parts)
```

### Verification checklist (run after every API push)

- [ ] All book/product titles render on the page (not just in CMS editor)
- [ ] Images display centered at intended size
- [ ] Internal and external links are clickable
- [ ] No orphaned sections (headings without body text below them)
- [ ] Content on rendered page matches the source markdown
