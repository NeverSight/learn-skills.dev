---
name: easy-wireframe
description: 生成产品原型 / 页面流程线框图，移动端(App)与 PC(桌面 Web)两种形态。当用户想做产品原型、线框图、页面流程图、交互流程、App 原型、后台/管理系统原型、把多个页面平铺出来用箭头标注点击流转时使用。产出单文件 HTML：移动端用 iOS 标准尺寸(375×812)+Apple HIG 字号，PC 用桌面尺寸(1440×900)+更密的 Web 字号；同为极简黑白灰线框风格，内置标注系统与整页截图导出。
---
