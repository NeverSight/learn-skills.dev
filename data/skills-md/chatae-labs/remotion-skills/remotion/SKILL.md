---
name: remotion
description: "Remotion video creation in React: composition authoring, animation patterns, and rendering. Use when creating, editing, or rendering Remotion videos. Covers GPU-accelerated rendering via Modal and local rendering fallback."
---
...
4. Output path discipline: Render directly to the session directory output path. Call yield_output with changeType: "new" on first render and changeType: "updated" on revisions — not "new" each time.

5. Confirm render complete before reporting done: Don't report the video as finished until the render script has exited successfully. Check the output file exists before calling yield_output.

6. Keep compositions short and self-contained: For simple animations (text, transitions), keep everything in one .tsx file. Avoid splitting into multiple components unless the user requests complexity.
