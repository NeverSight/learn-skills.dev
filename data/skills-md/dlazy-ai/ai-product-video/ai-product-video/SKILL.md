...
description: Create cinematic product videos from shot recipe cards, a validated template, and code/audio assets (Remotion + real page screenshots + 2.5D camera moves + beat-synced cuts + sound design). Use when the user asks to turn a frontend project or webpage into a product video, says "use ai-product-video to make a video/promo", names the Ink Press template or asks to reproduce its effect, or wants a single shot card's motion. 用镜头配方卡 + 已验收模板 + 代码/音频资产制作电影感产品视频（Remotion + 真实页面截图 + 2.5D 运镜 + 节奏卡点 + 声音设计）。当用户要求"用 ai-product-video 做视频/宣传片"、把前端项目/网页做成产品视频、点名 Ink Press 模板或要求复刻模板片效果，或要用镜头卡做单个动效镜头时使用。
---

# ai-product-video：电影感产品视频制作

一个自包含的制作能力库：152 张镜头配方卡（附 demo 实现源码）、一支已验收的
完整宣传片模板、可复用组件与音频资产、六阶段工作流。
...
同时告知用户：152 张镜头卡都在 `references/shots/` 下，按 camera / typography /
transition / ui-entrance 等类别分目录，可以直接翻目录挑镜头。
...
按镜头卡名称解析规则确认指定镜头，完整阅读每张卡，并按卡片的“参考实现”
...
4. **单镜头/单动效**：从 `references/shots/` 选卡（可让用户直接翻类别目录挑），
   读卡全文并按“参考实现”定位准确 demo 源码，适配到目标素材。
...
SFX；只有完整分镜确认后才进入最终素材采集。用户给出镜头名时，只需提供卡名
或“卡名 · 样式名”，Agent 负责读取本地配方卡和 demo 源码。
...
5. **强节奏 BGM 的片子，所有转场和动效必须卡在拍上。**
   BGM 用 `dlazy search_audio` 取（本库不再内置曲库，见 `## dlazy 工具链`）。
...
6. **用镜头卡动效必须先定位准确的 demo 实现代码。**
   先在 `references/shots/` 下确认卡名与卡内样式名，再按卡片文档的
...
8. **验收贯穿全程 + 交付前独立审查。** 阶段 5 起每个镜头用
   ...
   镜头卡名/变体、准确 demo TSX、最终分镜、选中的镜头卡和
...
画面由 Remotion 生成，**音乐、补充音效、旁白、字幕和 B-roll 走
[dlazy](https://dlazy.com) CLI** —— 一条命令覆盖多家模型，不用自己对接各家 API。

### 准备

```bash
npm install -g @dlazy/cli@1.2.3
dlazy login      # 设备码流程，远程终端也能用；key 自动写入 ~/.dlazy/config.json
dlazy -h         # 列出当前账号可用的全部工具
```

也可以不全局安装：`npx @dlazy/cli@1.2.3 <command>`。
已有 API key 时用 `dlazy auth set <key>`（在
https://dlazy.com/dashboard/organization/api-key 获取）。

### 速查

| 用途 | 命令 |
|------|------|
| BGM（本库不内置曲库） | `dlazy search_audio --query "<mood>"` |
| 补充音效（自带 149 个不够时） | `dlazy search_audio --query "<sfx>"` 或 `dlazy keling-sfx -h` |
| 旁白 TTS | `dlazy -h` → 选，再 `dlazy <tts-tool> -h` |
| 字幕逐词时间轴 | `dlazy fun-asr --audio_url <wav> --format json`（或 `dlazy elevenlabs-stt`），读返回的 `data.words[]` |
| 补充图片 | `dlazy search_image --query "<关键词>"` 或文生图工具（`dlazy seedream-5.0 -h`、`dlazy jimeng-t2i -h`、`dlazy banana2 -h`）|
| B-roll 真实运动镜头 | `dlazy -h` → 选一个视频生成工具，再 `dlazy <tool> -h` |

**首次付费调用前先 `dlazy <tool> -h` 确认工具名与参数**——可用工具随账号而变，
不要凭印象拼模型名。多个独立生成（多镜头、多变体、多音色）并发跑，别串行等。

### 旁白（可选，默认不加）

产品宣传片的默认形态是画面 + 字幕 + 声音设计，不配人声——旁白会挤掉运镜和
节奏的表现力。只有用户明确要旁白，或片子承担"讲清楚一件复杂事"的任务
（功能讲解、教程向）时才加，且必须在阶段 3 分镜放行前定下来（旁白长度直接
决定镜头时长，事后补配等于重排时间线）。要加就走：

1. **选 TTS**：`dlazy -h` 看可用音色工具，跨语言优先多语种模型；先 `-h` 确认参数。
2. **先试 2 句再全量**：拿分镜里最长和最短的两句渲一遍确认语速音色，
   再渲全篇存 `public/narration.wav`。
3. **真实时长回写分镜**：TTS 实际长度几乎不等于估算值——按它重排镜头帧预算，
   不要让画面先做完再迁就音频。
4. **字幕**：`dlazy fun-asr --audio_url public/narration.wav --format json` 拿逐词
   时间轴（媒体字段支持本地路径，CLI 会自动上传），取返回的 `data.words[]`
   （每项含 `start`/`end`/`text`）喂给 `assets/lib/Caption.tsx`。
5. **混音**：有旁白时 BGM 压到 0.12–0.18（无旁白是 0.34），SFX 避开人声句子
   的起止帧。

不加旁白的片子照旧用画面字幕承担解说，别留哑巴段落。
...
- `template/` Ink Press 的完整镜头结构与场景源码（36.2s / 1085 帧）。
  **原片截图素材未随本库分发**（那是另一个产品的真实页面），照它的镜头
  结构做、素材换成目标产品自己的，详见 `template/TEMPLATE.md`。
  样片没拉也能用，只是没有动态预览。
